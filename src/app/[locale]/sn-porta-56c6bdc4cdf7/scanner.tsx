...
export function Scanner({ initialPin }: { initialPin: string }) {
  ...
  const handleInput = useCallback(
    async (parsed: ParsedInput) => {
      ...
      if (!parsed.token && parsed.serial === undefined) {
        setErrorMsg(copy.door.invalidCode);
        setMode("error");
        return;
      }
    ...
    [copy.common.unknownError, copy.door.invalidCode, pin, stopCamera],
  );
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message : copy.door.cameraError);
    }
  }, [copy.door.cameraError, handleInput]);
  ...
  async function submitManual(e: React.FormEvent) {
    ...
    if (!parsed.token && parsed.serial === undefined) {
      setErrorMsg(copy.door.invalidFormat);
      return;
    }
    ...
    return (
      <main style={styles.main}>
        <div style={styles.box}>
          ...
          <h1 style={styles.h1}>{copy.common.doorTitle}</h1>
          <form onSubmit={submitPin} style={styles.form}>
            <label style={styles.label}>
              <span style={styles.labelText}>{copy.door.pin}</span>
              <input
                type="password"
                required
                value={pin}
      ...
      <div style={styles.box}>
              ...
              {!cameraActive && (
                <div style={styles.videoPlaceholder}>
                  <button onClick={startCamera} style={styles.button}>
                    {copy.door.enableCamera}
                  </button>
                </div>
              )}
            </div>

            <div style={styles.divider}>{copy.door.or}</div>

            <form onSubmit={submitManual} style={styles.form}>
              <label style={styles.label}>
                <span style={styles.labelText}>{copy.door.manualLabel}</span>
                <input
                  type="text"
                  value={manualInput}
                  onChange={(e) => setManualInput(e.target.value)}
                  placeholder={copy.door.manualPlaceholder}
                  autoComplete="off"
                  autoCapitalize="off"
                  spellCheck={false}
                  style={styles.input}
        ...
        {mode === "peek" && ticket && (
          <div>
            ...
            >
              {ticket.checkedInAt
                ? copy.door.alreadyAdmittedAt(
                    formatTime(ticket.checkedInAt, locale),
                  )
                : copy.door.readyToAdmit}
            </div>
            <div style={styles.serial}>{ticket.serialCode}</div>
            <div style={styles.name}>{ticket.name}</div>
            <div style={styles.seats}>
              {copy.common.seatCount(ticket.seats)}
            </div>
          ...
          <div>
            ...
            >
              {result.alreadyCheckedIn
                ? copy.door.alreadyAdmittedAt(
                    formatTime(result.checkedInAt, locale),
                  )
                : copy.door.admitted}
            </div>
            ...
            <div style={styles.seats}>
              {copy.common.seatCount(result.seats)}
            </div>
...
function formatTime(iso: string, locale: AppLocale) {
...
function ReservationBanner({
  ...
  return (
    <div style={styles.reservedBanner}>
      <div style={styles.reservedLabel}>{label}</div>

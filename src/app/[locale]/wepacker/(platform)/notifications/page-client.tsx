"use client";

import { useState } from "react";
import { useLocale } from "next-intl";
import { wp } from "@/i18n/copy/wepacker";
import { useRouter } from "@/i18n/navigation";
import {
  markAllNotificationsRead,
  markNotificationRead,
  retryNotificationEmail,
  type NotificationView,
} from "@/lib/wepacker/actions/notification";
import { NOTIFICATION_COPY } from "@/lib/wepacker/notification-copy";

const PORTUGUESE_NOTIFICATION_COPY: typeof NOTIFICATION_COPY = {
  pack_invited: {
    title: "Convite para Pack",
    body: "Recebeste um convite separado para uma comunidade.",
  },
  pack_accepted: {
    title: "Convite para Pack aceite",
    body: "Uma pessoa aceitou o teu convite para a comunidade.",
  },
  connection_requested: {
    title: "Pedido de Connection",
    body: "Recebeste um pedido separado de Connection.",
  },
  connection_accepted: {
    title: "Connection aceite",
    body: "O teu pedido de Connection foi aceite.",
  },
  mentorship_invited: {
    title: "Convite para Mentorship",
    body: "Recebeste um convite para uma Mentorship.",
  },
  mentorship_accepted: {
    title: "Mentorship aceite",
    body: "O teu convite para a Mentorship foi aceite.",
  },
  session_scheduled: {
    title: "Session agendada",
    body: "Foi adicionada uma nova Session ao teu calendário.",
  },
  session_updated: {
    title: "Session atualizada",
    body: "O horário ou o link de reunião de uma Session mudou.",
  },
  session_cancelled: {
    title: "Session cancelada",
    body: "Uma Session agendada foi cancelada.",
  },
  session_followup_updated: {
    title: "Seguimento da Session atualizado",
    body: "O organizador da Session atualizou o seguimento visível aos participantes.",
  },
  new_message: {
    title: "Nova Message",
    body: "Recebeste uma nova Message.",
  },
};

function formatWhen(value: string, locale: string): string {
  return new Intl.DateTimeFormat(locale, {
    dateStyle: "medium",
    timeStyle: "short",
  }).format(new Date(value));
}

function deliveryStatus(
  notification: NotificationView,
  locale: string,
): string | null {
  if (
    !notification.email ||
    notification.email.status === "sent" ||
    notification.email.status === "superseded"
  ) {
    return null;
  }
  if (notification.email.status === "failed") {
    return notification.email.canRetry
      ? wp(
          locale,
          "O envio do email falhou — podes tentar novamente.",
          "Email delivery failed — retry available.",
        )
      : wp(
          locale,
          "O envio do email falhou depois de todas as tentativas.",
          "Email delivery failed after all retries.",
        );
  }
  return wp(
    locale,
    "Envio do email pendente.",
    "Email delivery pending.",
  );
}

export default function NotificationsPageClient({
  notifications,
}: {
  notifications: NotificationView[];
}) {
  const locale = useLocale();
  const router = useRouter();
  const [busyId, setBusyId] = useState<string | null>(null);
  const [markingAll, setMarkingAll] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const unreadCount = notifications.filter((item) => !item.readAt).length;

  async function openNotification(notification: NotificationView) {
    setBusyId(notification.id);
    setError(null);
    try {
      if (!notification.readAt) await markNotificationRead(notification.id);
      router.push(notification.href);
      router.refresh();
    } catch {
      setError(
        wp(
          locale,
          "Não foi possível abrir esta Notification.",
          "Could not open this Notification.",
        ),
      );
    } finally {
      setBusyId(null);
    }
  }

  async function markAllRead() {
    setMarkingAll(true);
    setError(null);
    try {
      await markAllNotificationsRead();
      router.refresh();
    } catch {
      setError(
        wp(
          locale,
          "Não foi possível atualizar as Notifications.",
          "Could not update the Notifications.",
        ),
      );
    } finally {
      setMarkingAll(false);
    }
  }

  async function retryEmail(notificationId: string) {
    setBusyId(notificationId);
    setError(null);
    try {
      await retryNotificationEmail(notificationId);
      router.refresh();
    } catch {
      setError(
        wp(
          locale,
          "Não foi possível repetir o envio.",
          "Could not retry the delivery.",
        ),
      );
    } finally {
      setBusyId(null);
    }
  }

  return (
    <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-barlow text-2xl font-bold text-wepac-white">
            Notifications
          </h1>
          <p className="mt-1 text-sm text-wepac-text-tertiary">
            {unreadCount === 0
              ? wp(
                  locale,
                  "Sem Notifications por ler.",
                  "No unread Notifications.",
                )
              : wp(
                  locale,
                  `${unreadCount} Notification${unreadCount === 1 ? "" : "s"} por ler.`,
                  `${unreadCount} unread Notification${unreadCount === 1 ? "" : "s"}.`,
                )}
          </p>
        </div>
        {unreadCount > 0 && (
          <button
            type="button"
            disabled={markingAll}
            onClick={markAllRead}
            className="border border-wepac-border px-3 py-2 text-xs text-wepac-text-secondary disabled:opacity-50"
          >
            {wp(locale, "Marcar todas como lidas", "Mark all as read")}
          </button>
        )}
      </div>

      {error && (
        <p role="alert" className="mt-4 text-sm text-wepac-error">
          {error}
        </p>
      )}

      <div className="mt-6 border border-wepac-border bg-wepac-card">
        {notifications.length === 0 ? (
          <p className="p-6 text-sm text-wepac-text-tertiary">
            {wp(
              locale,
              "Convites de relações, alterações a Sessions, notas partilhadas e novas Messages aparecem aqui.",
              "Relationship invitations, Session changes, shared notes, and new Messages will appear here.",
            )}
          </p>
        ) : (
          notifications.map((notification) => {
            const copy = wp(
              locale,
              PORTUGUESE_NOTIFICATION_COPY[notification.type],
              NOTIFICATION_COPY[notification.type],
            );
            const status = deliveryStatus(notification, locale);
            return (
              <article
                key={notification.id}
                className={`border-b border-wepac-border p-5 last:border-b-0 ${
                  notification.readAt ? "opacity-70" : "bg-wepac-white/[0.03]"
                }`}
              >
                <div className="flex items-start gap-3">
                  <span
                    aria-hidden="true"
                    className={`mt-1.5 h-2 w-2 flex-none rounded-full ${
                      notification.readAt
                        ? "bg-wepac-text-tertiary"
                        : "bg-wepac-white"
                    }`}
                  />
                  <div className="min-w-0 flex-1">
                    <h2 className="font-barlow text-base font-bold text-wepac-white">
                      {copy.title}
                    </h2>
                    <p className="mt-1 text-sm text-wepac-text-secondary">
                      {copy.body}
                    </p>
                    <p className="mt-2 text-xs text-wepac-text-tertiary">
                      {formatWhen(notification.createdAt, locale)}
                    </p>
                    {status && (
                      <p className="mt-2 text-xs text-wepac-warning">{status}</p>
                    )}
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        type="button"
                        disabled={busyId === notification.id}
                        onClick={() => openNotification(notification)}
                        className="bg-wepac-white px-3 py-1.5 text-xs font-bold text-wepac-black disabled:opacity-50"
                      >
                        {wp(locale, "Abrir", "Open")}
                      </button>
                      {notification.email?.canRetry && (
                        <button
                          type="button"
                          disabled={busyId === notification.id}
                          onClick={() => retryEmail(notification.id)}
                          className="border border-wepac-border px-3 py-1.5 text-xs text-wepac-text-secondary disabled:opacity-50"
                        >
                          {wp(locale, "Repetir email", "Retry email")}
                        </button>
                      )}
                    </div>
                  </div>
                </div>
              </article>
            );
          })
        )}
      </div>
    </div>
  );
}

...
import { formatPriceCents } from "@/app/[locale]/bilheteira/ui";
import { footerSocialLinks } from "@/data/arte-a-capela";
import type { Metadata } from "next";
import { getLocale } from "next-intl/server";
import { LocaleSwitcher } from "@/components/LocaleSwitcher";
import { getArteACapelaCopy } from "@/i18n/copy/institutional-arte-a-capela";
...
export async function generateMetadata(): Promise<Metadata> {
  const copy = getArteACapelaCopy(await getLocale());
  return {
...
function formatDateFull(d: Date, locale: string): string {
  return new Intl.DateTimeFormat(locale, {
    timeZone: LISBON_TZ,
    day: "2-digit",
    month: "long",
...
export default async function ArteACapelaPage({ searchParams }: Props) {
  ...
  const copy = getArteACapelaCopy(locale);
  ...
  const event: EventView | null = dbEvent
    ? {
        ...
        dateShort: formatDateShort(dbEvent.startsAt, locale),
        dateFull: formatDateFull(dbEvent.startsAt, locale),
        timeLabel: formatTime(dbEvent.startsAt, locale),
        doorsLabel: dbEvent.doorsAt
          ? formatTime(dbEvent.doorsAt, locale)
          : null,
        venue: dbEvent.venue,
        address: dbEvent.address,
        priceLabel:
          cheapestTierPriceCents !== null
            ? formatPriceCents(cheapestTierPriceCents, locale)
            : "—",
      }
    : null;
  ...
  return (
    <div className="bg-capela-bg text-white overflow-x-hidden">
        ...
        <div className="relative z-10 w-full max-w-[1600px] mx-auto px-6 md:px-10 lg:px-16 xl:px-24 pt-[110px] lg:pt-[130px] pb-16 grid lg:grid-cols-[auto_360px] gap-10 lg:gap-14 lg:justify-center items-end">
          ...
          <div className="relative space-y-6 lg:space-y-8">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row lg:flex-col items-start gap-3 sm:gap-4">
              ...
              >
                {event ? copy.buyTicket : copy.viewTickets}
              </a>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[1.41fr_1fr_1.01fr]">
        {["claustro.jpg", "talha.jpg", "vitral.jpg"].map((src, index) => (
          <div
            key={src}
            className="relative h-[280px] sm:h-[380px] lg:h-[520px]"
          >
            <img
              src={`/images/arte-a-capela/${src}`}
            ...
            {event ? (
              <>
                ...
                <dl className="space-y-4">
                  {[
                    ...
                    ...(event.doorsLabel
                      ? [{ label: copy.doors, value: event.doorsLabel }]
                      : []),
                  ].map((row) => (
                    <div
                      key={row.label}
              ...
              <>
                <h2 className={`${serif} text-[32px] sm:text-[54px] leading-[1.15] mb-4`}>
                  {copy.noUpcomingEvent.ticketingHeading}
                </h2>
                    ...
                    <li key={tier.id} className="py-8 first:pt-0">
                      <div className="flex items-baseline justify-between gap-6">
                        ...
                        <span className="text-[20px] font-medium whitespace-nowrap">
                          {formatPriceCents(tier.priceCents, locale)}
                        </span>
                      </div>
                      {tier.description && (

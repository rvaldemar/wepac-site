...
function formatTime(d: string | Date, locale: string): string {
...
async function lookupTicket(
  ...
  if (!res.ok) {
    ...
    throw new Error((body as { error?: string }).error || errorMessage);
  }
  return res.json();
}
...
export function CheckinScanner({
  ...
  const copy = getTicketAdminCopy(locale).checkin;
  ...
  const handleTicketFound = useCallback(
    async (raw: string) => {
      ...
      try {
        const ticket = await lookupTicket(raw, copy.lookupError);
    ...
    [copy.actionError, copy.lookupError, resetScanner]
  );
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      setScanResult({ state: "error", message: copy.cameraError });
    }
  }, [copy.cameraError, handleTicketFound]);
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", fontFamily: "Inter, sans-serif" }}>
      ...
      >
        <span style={{ fontWeight: 700 }}>
          {copy.admittedCount(admittedCount)}
        </span>
        <span style={{ opacity: 0.7 }}>
          {copy.totalTickets(totalTickets)}
        </span>
      </div>
...
function TicketResultCard({
  ...
  isPending: boolean;
  locale: string;
  copy: ReturnType<typeof getTicketAdminCopy>["checkin"];
}) {
    ...
    >
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 13, color: "#555", marginTop: 2 }}>
          ...
          {copy.seatCount(ticket.seats)}
        </div>
        {ticket.checkedInAt && (
          <div style={{ fontSize: 12, color: "#666", marginTop: 4 }}>
            {copy.admittedAt(formatTime(ticket.checkedInAt, locale))}
          </div>
        )}
      </div>
        ...
        <div>
          ...
          {ticket.checkLogs.map((log, i) => (
            ...
            >
              ...
              {copy.at} {formatTime(log.createdAt, locale)}
            </div>
          ))}
        </div>
      )}

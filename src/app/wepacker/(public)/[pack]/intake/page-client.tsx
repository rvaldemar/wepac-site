...
function validateFields(fd: FormData, variant: "artist" | "generic"): FieldErrors {
...
export function CandidaturaFormClient({
  packSlug,
  variant = "artist",
  source,
  ...
  const isArtisticBranch =
    !isGeneric || entrantType === "artist" || focus === "artistic-practice";
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const errors = validateFields(fd, variant);
    ...
    const data = {
      ...
      area: isArtisticBranch ? (fd.get("area") as string) || undefined : undefined,
      socialLinks: isArtisticBranch
        ? (fd.get("socialLinks") as string) || undefined
        : undefined,
      motivation: isGeneric
        ? buildGenericMotivation(fd, source)
        : (fd.get("motivation") as string) || undefined,
    };
    ...
    return (
      ...
      >
        <p className="font-barlow text-2xl font-bold text-wepac-white">
          {isGeneric ? "Ponto de partida recebido" : "Candidatura recebida"}
        </p>
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          name="motivation"
          required={isGeneric}
          aria-required={isGeneric ? "true" : undefined}
          ...
          placeholder={
            isGeneric
              ...
              : "Conta-nos em poucas palavras (máx. 500 caracteres)"
          }
          maxLength={500}
          rows={4}
      ...
      <p className="text-xs text-wepac-text-tertiary">
        Os dados que preenches aqui são usados apenas para{" "}
        {isGeneric
          ? "compreender o teu ponto de partida e entrar em contacto contigo"
          : "responder à tua candidatura"}
        . Consulta a{" "}

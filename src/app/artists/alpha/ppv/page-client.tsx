...
import { upsertLifePlan } from "@/lib/actions/strategic";
...
export default function PPVPageClient({ userId, plan }: Props) {
 const defaultValues: Record<SectionKey, string> = {
  whoIAm: plan?.whoIAm ?? "",
 ...
 const [values, setValues] = useState<Record<SectionKey, string>>(defaultValues);
     ...
     return (
      <div key={section.key} className="border border-wepac-border bg-wepac-card p-6">
       <div className="flex items-start justify-between">
        ...
        <button
         onClick={() => isEditing ? handleSave(section.key) : setEditing(section.key)}

...
interface Props {
 plan: {
  ...
  positioning: string;
  focusAreas: AreaKey[];
  quarterlyReflection: string | null;
  goals: Goal[];
  monthlyActions: MonthlyAction[];
 } | null;
}

export default function PlanPageClient({ plan }: Props) {
  ...
  return (
   <div className="p-6 lg:p-8">
    ...
    <p className="mt-4 text-sm text-wepac-text-tertiary">
     Ainda sem plano estratégico definido.
    </p>
   </div>
  );
 }
      ...
      {annualGoals.map((goal) => (
       <div key={goal.id} className="border border-wepac-border bg-wepac-card p-5">
        <div className="flex items-start justify-between">
         ...
         <span className={`px-2 py-0.5 text-xs ${STATUS_COLORS[goal.status]}`}>
      ...
      <div className="border border-wepac-border bg-wepac-card p-5">
       ...
       <div className="mt-2 flex gap-2">
        {plan.focusAreas.map((a) => (
         <span
          key={a}
          className="bg-wepac-white/10 px-3 py-1 text-xs text-wepac-white"
         >
          {AREA_LABELS[a]}
         </span>
        ))}
      ...
      <div>
       ...
       <div className="mt-3 space-y-3">
        {quarterlyGoals.map((goal) => (
         <div key={goal.id} className="border border-wepac-border bg-wepac-card p-4">
          <div className="flex items-start justify-between">
      ...
      {plan.monthlyActions
       ...
       .map((action) => (
        <div key={action.id} className="flex items-center justify-between border border-wepac-border bg-wepac-card p-4">
         ...
         <span className={`px-2 py-0.5 text-xs ${STATUS_COLORS[action.status]}`}>
